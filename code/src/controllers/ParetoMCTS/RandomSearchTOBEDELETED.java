 ...
 * Date: 17/10/12
 */
public class RandomSearchTOBEDELETED
{
    /**
     ...
     */
    public RandomSearchTOBEDELETED()
    {
        m_rnd = new Random();
    ...
    public double createRandomPath(long a_timeDue)
    {
        ...
        for(int i = 0; !end && i < m_currentRandomPath.length; ++i)
        {
            ...
            for(int j =0; !end && j < RandomSearchTOBEDELETED.MACRO_ACTION_LENGTH; ++j)
            {
        ...
        return RandomSearchTOBEDELETED.scoreGame();
    }
    ...
    public static void updateNextWaypoints(int a_howMany)
    {
        ...
        try{
            ...
            if(nVisited != waypoints.size())
            {
                ...
                int bestPath[] = ParetoMCTSController.m_tspGraph.getBestPath();
    ...
    public static double scoreGame()
    {
        ...
        }else
        {
            ...
            if(match(m_futureGameState.getVisitOrder(), ParetoMCTSController.m_tspGraph.getBestPath()))
            {
                if(obj0Collected)
    ...
    private static Path getPathToGameObject(Game a_game, GameObject a_gObj, int a_objKey)
    {
        ...
        if(RandomSearchTOBEDELETED.m_nodeLookup.containsKey(a_objKey))
            objectNode = RandomSearchTOBEDELETED.m_nodeLookup.get(a_objKey);
        else{
            objectNode = ParetoMCTSController.m_graph.getClosestNodeTo(a_gObj.s.x, a_gObj.s.y);
            RandomSearchTOBEDELETED.m_nodeLookup.put(a_objKey, objectNode);
        }
