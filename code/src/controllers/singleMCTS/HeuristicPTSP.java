 ...
 * Time: 17:07
 */
public class HeuristicPTSP implements Heuristic
{ /** TUNABLE PARAMETERS **/
public double ALPHA = 0.9;
    ...
    {
        MACRO_ACTION_LENGTH = SingleMCTSParameters.MACRO_ACTION_LENGTH;
        ROLLOUT_DEPTH = SingleMCTSParameters.ROLLOUT_DEPTH;
    ...
    public double value(Game a_gameState)
    {
        ...
        if((!matching) || (a_gameState.isEnded() && a_gameState.getWaypointsLeft()>0))
        {
            ...
            return -2; // Game finished - game over.
        }
        ...
        double fuelPower = fuelPoints*SingleMCTSController.FUEL_POWER_MULT + distancePoints*(1.0-SingleMCTSController.FUEL_POWER_MULT);
        ...
        if(m_currentShipSpeed > SingleMCTSController.THRESHOLD_HIGH_SPEED)
            damagePower = damagePoints*SingleMCTSController.DAMAGE_POWER_MULT +
                    distancePoints*(1.0-SingleMCTSController.DAMAGE_POWER_MULT);
        else
            damagePower = damagePoints*SingleMCTSController.DAMAGE_POWER_MULT_SLOW +
       ...
       // double allInOne = distancePoints*0.6 + fuelPower*0.3 + damagePower*0.1;
        ...
        double allInOne = distancePoints;
        return allInOne;
    }
    ...
    private Path getPathToGameObject(Game a_game, GameObject a_gObj, int a_objKey)
    {
        ...
        Node shipNode = SingleMCTSController.m_graph.getClosestNodeTo(a_game.getShip().s.x, a_game.getShip().s.y);
        ...
        if(m_nodeLookup.containsKey(a_objKey))
            ...
            objectNode = SingleMCTSController.m_graph.getClosestNodeTo(a_gObj.s.x, a_gObj.s.y);
            m_nodeLookup.put(a_objKey, objectNode);
        }

        //Get the parh between the nodes.
        return SingleMCTSController.m_graph.getPath(shipNode.id(), objectNode.id());
    }
    ...
    private Path getPathBetweenGameObjects(GameObject a_gObj, int a_objKey, GameObject a_gObj2, int a_objKey2)
    {
        ...
        if(m_nodeLookup.containsKey(a_objKey))
            ...
            object1Node = SingleMCTSController.m_graph.getClosestNodeTo(a_gObj.s.x, a_gObj.s.y);
        ...
        if(m_nodeLookup.containsKey(a_objKey2))
            ...
            object2Node = SingleMCTSController.m_graph.getClosestNodeTo(a_gObj2.s.x, a_gObj2.s.y);
            m_nodeLookup.put(a_objKey2, object2Node);
        }
        //Get the parh between the nodes.
        return SingleMCTSController.m_graph.getPath(object1Node.id(), object2Node.id());
    }
