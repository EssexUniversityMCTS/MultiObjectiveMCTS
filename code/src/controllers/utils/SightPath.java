...
public class SightPath
{
    ...
    public int m_numLavaPositions;
    public double m_percLava;
    ...
    public SightPath(Path a_p, Graph a_graph, Game a_game)
    {
        ...
        for(Integer point : a_p.m_points)
        {
            Node thisNode =  a_graph.getNode(point);
            if(a_game.getMap().isLava(thisNode.x(), thisNode.y()))
                m_numLavaPositions++;
        }

        m_percLava = (double)m_numLavaPositions/p.m_points.size();
