...
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=2.1\columnwidth]{img/dstAll}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Results in DST: percentages of each optima found during $100$ games played with different weight vectors. Scalarization approaches converge to the edges of the optimal front, whereas Pareto approaches are able to find all optimal solutions. The proposed algorithm, MO-MCTS, finds these solutions significantly more often than NSGA-II.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:extCase}
%DIFDELCMD < \end{figure*}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Figure~\ref{fig:extCase} shows the results obtained after these experiments were performed. The first point to note is that MCTS only converges (mostly) to the two optimal points located at the edges of the optimal Pareto front (\textit{OPT-1} and \textit{OPT-10}, see also Figure~\ref{fig:optDst}). This is an expected result, as K. Deb. suggested in~\cite{Deb2001} and was also discussed before in Section~\ref{sec:moo}: linear scalarization approaches only converge to the edges of the optimal Pareto front if its shape is non-convex.
...
\hline
 ...
 & $(0.6,0.1,0.3)$ & \DIFdelbeginFL %DIFDELCMD < \bm{$1649 \pm 8$} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1649 \pm 8$ }\DIFaddendFL & $132 \pm 2$ & $833 \pm 13$\\
\hline
...
\hline
 ...
 & $(0.6,0.1,0.3)$ & \DIFdelbeginFL %DIFDELCMD < \bm{$1399 \pm 9$} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1399 \pm 9$ }\DIFaddendFL & $469 \pm 4$ & $547 \pm 3$\\
\hline
 ...
 & $(0.6,0.1,0.3)$ & \DIFdelbeginFL %DIFDELCMD < \bm{$1571 \pm 7$} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1571 \pm 7$ }\DIFaddendFL & $559 \pm 5$ & $294 \pm 4$\\
\hline
