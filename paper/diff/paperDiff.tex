...
Secondly, the algorithm must be able to provide solutions across the multi-objective spectrum: by parametrizing the algorithm, it must be possible to prioritize one objective over the others and therefore converge to solutions according to these preferences. \DIFaddbegin \DIFadd{Therefore, the main contribution of this paper is the proposal of a multi-objective version of MCTS that is applicable to real-time domains and is able to provide different solutions across the multi-objective spectrum.
}\DIFaddend 
...
Monte Carlo Tree Search (MCTS) is a tree search algorithm that was originally applied to board games, gaining momentum and popularity in the game of Go. This game is played in a square grid board, with a size of $19 \times 19$ in the original game, and $9 \times 9$ in its reduced version. The game is played in turns, and the objective is to surround the opponent's stones by placing stones in any available position on the board. Due to the very large branching factor of \DIFdelbegin \DIFdel{Go, this game is considered the drosophila of Game AI, and MCTS players have reached }\DIFdelend \DIFaddbegin \DIFadd{the game and the absence of clear heuristics to tackle it, Go became object of study for many researchers. MCTS was the first algorithm able to reach }\DIFaddend professional level play in the reduced board size version~\cite{Lee2009}. After its success in Go, MCTS has been used extensively by many researchers in this and different domains. An extensive survey of MCTS methods, variations and applications, has been written by Browne et al.~\cite{Browne2012}. 
...
MCTS has also been applied to single-player games, like SameGame~\DIFdelbegin %DIFDELCMD < \cite{Matsumoto2010}%%%
\DIFdelend \DIFaddbegin \cite{Schadd2008}\DIFaddend , where the player's goal is to destroy contiguous tiles of the same colour, distributed in a rectangular grid. Another use of MCTS is in the popular puzzle Morpion Solitaire~\DIFdelbegin %DIFDELCMD < \cite{Edelkamp2010}%%%
...
In this case, it is said that these solutions are \DIFdelbegin \DIFdel{non-dominated with respect }\DIFdelend \DIFaddbegin \DIFadd{indifferent }\DIFaddend to each other. Solutions that are not dominated can be grouped in a \textit{non-dominated set}. Given a non-dominated set $P$, it is said that $P$ is the \textit{Pareto-set} if there is no other solution in the decision space that dominates any member of $P$. The corresponding objective vectors of the members in the Pareto-set build a so called \textit{Pareto-front}. The relations between decision and objective space, dominance and the non-dominated set are depicted in Figure~\ref{fig:moop}.

\begin{figure} [!t]
	\begin{center}
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.235]{img/moopDec.png}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.235]{img/moopDec2.png}
	\DIFaddendFL \caption{Decision and objective spaces in a \DIFdelbeginFL \DIFdelFL{MOOP }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MOP }\DIFaddendFL with two variables ($x_1$ and $x_2$) and two objectives ($f_1$ and $f_2$). In the objective space, yellow dots are non-optimal objective vectors, while blue dots form a non-dominated set. \DIFaddbeginFL \DIFaddFL{From~}\cite{Brockhoff2013}\DIFaddFL{.}\DIFaddendFL }
	\label{fig:moop}
	\end{center}
\end{figure}

There are many different algorithms in the \DIFdelbegin \DIFdel{litrature which }\DIFdelend \DIFaddbegin \DIFadd{literature that }\DIFaddend are proposed to tackle multi-objective optimization problems~\cite{Deb2001}. \DIFdelbegin \DIFdel{Weighted-sum }\DIFdelend \DIFaddbegin \DIFadd{A weighted-sum }\DIFaddend approach is one of the traditional methods in which the objectives are weighted according to user preference. The sum of the weighted objectives builds one objective function \DIFdelbegin \DIFdel{which is }\DIFdelend to be optimized. The solution to this single-objective problem is one certain solution, ideally on a Pareto-front. By varying the weights, it is possible to converge to different optimal solutions. Such linear scalarization approaches fail to find optimal solutions for problems with non-convex-shape Pareto-fronts~\cite{Deb2001}.
...
One of the most well-known algorithms in the literature is the Non-dominated Sorting Evolutionary Algorithm II (NSGA-{II})~\cite{Deb2002}, \DIFaddbegin \DIFadd{the }\DIFaddend pseudocode of which is shown in Algorithm~\ref{alg:nsga2}. \DIFaddbegin \DIFadd{NSGA-II is known to deliver solutions with very good diversity and convergence to the Pareto-front for 2-objective problems. However, for problems with more than 3 objectives, other approaches such as NSGA-III~}\cite{Jain2013}\DIFadd{, SMS-EMOA~}\cite{Beume2007} \DIFadd{and MOEA/D~}\cite{Zhang2007a} \DIFadd{are shown to outperform NSGA-II. }\DIFaddend As in any evolutionary algorithm, NSGA-{II} evolves a set of $N$ individuals in a population denoted as $Pop$ with the difference that they are ranked according to \DIFaddbegin \DIFadd{a }\DIFaddend dominance criterion and a crowding distance measure\DIFdelbegin \DIFdel{which is }\DIFdelend \DIFaddbegin \DIFadd{, which are }\DIFaddend used to maintain a good diversity of solutions. \DIFaddbegin \DIFadd{After each iteration of the algorithm, only the best $N$ individuals according to this ranking are maintained to the next generation.
}\DIFaddend 
...
The three main pillars of the \DIFdelbegin \DIFdel{NSGA-2 }\DIFdelend \DIFaddbegin \DIFadd{NSGA-II }\DIFaddend algorithm are:
\begin{itemize}
...
\item Crowding distance: The function $\Call{crowdingDistanceAssignment}{F_i}$ assigns to each one of the individuals in $F_i$ a crowding distance value, which is the distance between the individual and its neighbours. The individuals with the smallest crowding distances are selected with a lower probability than the ones with \DIFdelbegin \DIFdel{the }\DIFdelend larger values.   
\item Elitism: The \DIFdelbegin \DIFdel{solutions in }\DIFdelend \DIFaddbegin \DIFadd{individuals from }\DIFaddend the first front $F_0$ \DIFaddbegin \DIFadd{are always ranked first, according to the dominance criterion and their crowding distance. This ensures that the best solutions }\DIFaddend always survive to the next \DIFdelbegin \DIFdel{population}\DIFdelend \DIFaddbegin \DIFadd{generation}\DIFaddend .
\end{itemize}
...
The proposed algorithm was employed successfully in two domains: the DST and the Grid Scheduling problem\DIFdelbegin \DIFdel{, matching state-of-the-art results in }\DIFdelend \DIFaddbegin \DIFadd{. In }\DIFaddend both cases, \DIFaddbegin \DIFadd{the results matched the state-of-the-art, albeit }\DIFaddend at the expense of a high computational cost.

As mentioned before, \DIFdelbegin \DIFdel{the objective }\DIFdelend \DIFaddbegin \DIFadd{one of the objectives }\DIFaddend of this paper is to propose an MO-MCTS algorithm that is suitable for real-time domains. \DIFdelbegin \DIFdel{Although the work discussed here is influenced by }\DIFdelend \DIFaddbegin \DIFadd{Therefore, an approach different from }\DIFaddend Wang's \DIFdelbegin \DIFdel{approach, some modifications need to be done }\DIFdelend \DIFaddbegin \DIFadd{is needed, }\DIFaddend in order to overcome the high computational cost involved in their approach.

In the algorithm proposed in this paper, the reward vector $\overline{r}$ that was obtained at the end of a Monte Carlo simulation is back-propagated through the nodes visited in the last iteration until the root is reached. In the vanilla algorithm, each node would use this vector $\overline{r}$ to update its own accumulated reward vector $\overline{R}$. In the algorithm proposed here, each node in the MO-MCTS algorithm also keeps a local Pareto front \DIFaddbegin \DIFadd{approximation }\DIFaddend ($P$), updated at each iteration with the reward vector $\overline{r}$ obtained at the end of the simulation. Algorithm~\ref{alg:updPareto} describes how the node statistics are updated in MO-MCTS.
...
Here, if $\overline{r}$ is not dominated by the local Pareto front \DIFaddbegin \DIFadd{approximation}\DIFaddend , it is added to the front and $\overline{r}$ is propagated to its parent. \DIFdelbegin \DIFdel{In case }\DIFdelend \DIFaddbegin \DIFadd{It may also happen that $\overline{r}$ dominates all or some solutions of the local front, in which case the new solution is included and the dominated solutions are removed from the Pareto front approximation. If }\DIFaddend $\overline{r}$ is dominated by the node's Pareto front \DIFdelbegin \DIFdel{, the local Pareto front }\DIFdelend \DIFaddbegin \DIFadd{approximation, it }\DIFaddend does not change and there is no need to \DIFdelbegin \DIFdel{keep }\DIFdelend \DIFaddbegin \DIFadd{maintain }\DIFaddend this propagation up the tree. Three observations can be made about the mechanism described here:
...
\item It is easy to infer, from the last point, that the Pareto front \DIFaddbegin \DIFadd{approximation }\DIFaddend of a node cannot be worse than the front of its children (in other words, the front of a child will never dominate that of its parent). Therefore, the root of the tree contains the best non-dominated front ever found during the search.
\end{itemize}
...
\DIFdelbegin \subsubsection{\DIFdel{Transposition Tables in DST}} %DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{ssec:transTables}
%DIFDELCMD < %%%
...
TT tables are implemented in the MCTS algorithms tested in this benchmark by using hash tables that store a \textit{representative} node for each pair $(position, moves)$ found.  The key of the hash map \DIFdelbegin \DIFdel{needs then }\DIFdelend \DIFaddbegin \DIFadd{then needs }\DIFaddend to be obtained from three values: position coordinates $x$ and $y$ of the ship in the board, and number of moves, indicated by the depth of the node in the tree. Hence, transpositions can only happen at the same depth within the tree, a feature that has been successfully tried before in the literature~\cite{Kozelek2009}.

\DIFdelbegin \subsubsection{\DIFdel{Heuristics for DST}} %DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sssec:heurDST}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend As DST has two different objectives, number of moves and value of the treasure, the quality of a state can be assessed by two rewards, $\rho_p$ and $\rho_v$, for each objective respectively. $\rho_p$ is adjusted to be maximized using the maximum number of moves in the game, while $\rho_v$ is \DIFdelbegin \DIFdel{just the value in the cell with }\DIFdelend \DIFaddbegin \DIFadd{simply the value of the cell that holds }\DIFaddend the treasure. \DIFaddbegin \DIFadd{Therefore, the reward vector to maximize is defined as $\overline{r} = \{ \rho_p, \rho_v\}$. }\DIFaddend Equation~\ref{eq:dst} summarizes these rewards:
...
\DIFdelbegin \subsubsection{\DIFdel{Macro-actions for MO-PTSP}} %DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sssec:macro}
%DIFDELCMD < %%%
...
\DIFdelbegin \subsubsection{\DIFdel{Heuristics for MO-PTSP}} %DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sssec:heurMOPTSP}
%DIFDELCMD < 
...
Finally, Equation~\ref{eq:dam} gives the method \DIFaddbegin \DIFadd{used }\DIFaddend to calculate the damage objective, $\rho_d$:
...
$g_t$ is the damage suffered so far in the game, and $g_M$ is the maximum damage the ship can take. In this case, three different variables are used to regulate the behaviour of this objective: $\gamma$, $\beta_{1}$ and $\beta_{2}$. Both $\beta_{1}$ and $\beta_{2}$ have the same role as $\alpha$ in Equation~\ref{eq:fuel}: they balance between the time objective and the damage measure. The difference is that $\beta_{1}$ is used in high speeds, while $\beta_{2}$ is employed with low velocities. This is distinguished by the parameter $\gamma$, that can be seen as a threshold value for the ship's speed ($sp$). This differentiation is made in order to avoid low speeds in lava lakes, as this \DIFdelbegin \DIFdel{increase }\DIFdelend \DIFaddbegin \DIFadd{significantly increases }\DIFaddend the damage suffered\DIFdelbegin \DIFdel{significantly}\DIFdelend . The values for these variables have also been determined empirically, and they are set to $\gamma = 0.8$, $\beta_{1} = 0.75$ and $\beta_{2} = 0.25$.

\DIFaddbegin \DIFadd{The final reward vector to be maximized is therefore $\overline{r} = \{ \rho_t, \rho_f, \rho_d \}$. }\DIFaddend It is important to highlight again that these three rewards \DIFdelbegin \DIFdel{, $\rho_t$, $\rho_f$ and $\rho_d$, }\DIFdelend are the same for \DIFdelbegin \DIFdel{the three }\DIFdelend \DIFaddbegin \DIFadd{all the }\DIFaddend algorithms tested in the experiments. 
...
The first point to check is if the different weight vectors affect the solutions obtained by MO-MCTS. Table~\ref{tab:weights} shows the results of executing the MO-MCTS controller during $30$ games in each one of the $10$ maps of the MO-PTSP, for every weight vector. \DIFaddbegin \DIFadd{This table includes the averages obtained and the independent t-test p-value for the results of the objective with the highest weight, compared with the result from the default weight vector $W_1 = (0.33,0.33,0.33)$ in the same map. }\DIFaddend It can be seen that the highest weight in $W$ leads, in most of the cases, to the best solution in that objective in the map. 


%DIF > \begin{table}[!t]
%DIF > \begin{center}
%DIF > %\begin{tabular}{|>{\centering\arraybackslash}m{0.65cm}|c|c|c|c|}
%DIF > \begin{tabular}{|p{0.8cm}|p{2.2cm}|m{1.13cm}|m{1.13cm}|m{1.13cm}|}
%DIF > \hline
%DIF > \textbf{Map}  & \textbf{$W:(w_t, w_f, w_d)$} & \textbf{Time} &  \textbf{Fuel} & \textbf{Damage} \\ 
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 1}} & $(0.33,0.33,0.33)$ & $1654 \pm 7$ & $131 \pm 2$& $846 \pm 13$\\
%DIF >  & $(0.1,0.3,0.6)$ & $1657 \pm 8$ & $130 \pm 2$ & \bm{$773 \pm 11$}\\
%DIF >  & $(0.1,0.6,0.3)$ & $1681 \pm 11$ & $131 \pm 2$ & $837 \pm 15$\\
%DIF >  & $(0.6,0.1,0.3)$ & $1649 \pm 8$ & $132 \pm 2$ & $833 \pm 13$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 2}} & $(0.33,0.33,0.33)$ & $1409 \pm 7$ & $235 \pm 4$& $364 \pm 3$\\
%DIF >  & $(0.1,0.3,0.6)$ & $1402 \pm 6$ & $236 \pm 5$ & \bm{$354 \pm 2$}\\
%DIF >  & $(0.1,0.6,0.3)$ & $1416 \pm 8$ & \bm{$219 \pm 4$} & $360 \pm 3$\\
%DIF >  & $(0.6,0.1,0.3)$ & $1396 \pm 8$ & $245 \pm 5$ & $361 \pm 2$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 3}} & $(0.33,0.33,0.33)$ & $1373 \pm 6$ & $221 \pm 3$& $301 \pm 7$\\
%DIF >  & $(0.1,0.3,0.6)$ & $1378 \pm 5$ & $211 \pm 3$ & \bm{$268 \pm 5$}\\
%DIF >  & $(0.1,0.6,0.3)$ & $1385 \pm 6$ & \bm{$203 \pm 4$} & $291 \pm 7$\\
%DIF >  & $(0.6,0.1,0.3)$ & \bm{$1363 \pm 4$} & $229 \pm 4$ & $285 \pm 7$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 4}} & $(0.33,0.33,0.33)$ & $1383 \pm 6$ & $291 \pm 5$& $565 \pm 5$\\
%DIF >  & $(0.1,0.3,0.6)$ & $1385 \pm 7$ & $304 \pm 4$ & \bm{$542 \pm 4$}\\
%DIF >  & $(0.1,0.6,0.3)$ & $1423 \pm 6$ & \bm{$273 \pm 4$} & $583 \pm 5$\\
%DIF >  & $(0.6,0.1,0.3)$ & $1388 \pm 6$ & $309 \pm 4$ & $559 \pm 5$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 5}} & $(0.33,0.33,0.33)$ & $1405 \pm 7$ & $467 \pm 4$& $559 \pm 4$\\
%DIF >  & $(0.1,0.3,0.6)$ & $1431 \pm 9$ & $447 \pm 4$ & $541 \pm 4$\\
%DIF >  & $(0.1,0.6,0.3)$ & $1467 \pm 9$ & \bm{$411 \pm 5$} & $567 \pm 5$\\
%DIF >  & $(0.6,0.1,0.3)$ & $1399 \pm 9$ & $469 \pm 4$ & $547 \pm 3$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 6}} & $(0.33,0.33,0.33)$ & $1575 \pm 7$ & $549 \pm 5$& $303 \pm 4$\\
%DIF >  & $(0.1,0.3,0.6)$ & $1626 \pm 9$ & $540 \pm 6$ & $286 \pm 5$\\
%DIF >  & $(0.1,0.6,0.3)$ & $1703 \pm 11$ & \bm{$499 \pm 4$} & $316 \pm 7$\\
%DIF >  & $(0.6,0.1,0.3)$ & $1571 \pm 7$ & $559 \pm 5$ & $294 \pm 4$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 7}} & $(0.33,0.33,0.33)$ & $1434 \pm 5$ & $599 \pm 6$& $284 \pm 6$\\
%DIF >  & $(0.1,0.3,0.6)$ & $1475 \pm 10$ & $602 \pm 5$ & \bm{$243 \pm 6$}\\
%DIF >  & $(0.1,0.6,0.3)$ & $1489 \pm 12$ & \bm{$549 \pm 3$} & $264 \pm 6$\\
%DIF >  & $(0.6,0.1,0.3)$ & \bm{$1407 \pm 8$} & $618 \pm 5$ & $270 \pm 6$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 8}} & $(0.33,0.33,0.33)$ & $1761 \pm 9$ & $254 \pm 5$& $382 \pm 3$\\
%DIF >  & $(0.1,0.3,0.6)$ & $1804 \pm 10$ & $269 \pm 4$ & \bm{$357 \pm 4$}\\
%DIF >  & $(0.1,0.6,0.3)$ & $1826 \pm 10$ & \bm{$230 \pm 3$} & $392 \pm 7$\\
%DIF >  & $(0.6,0.1,0.3)$ & \bm{$1732 \pm 9$} & $311 \pm 8$ & $379 \pm 6$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 9}} & $(0.33,0.33,0.33)$ & $2501 \pm 14$ & $926 \pm 6$& $574 \pm 9$\\
%DIF >  & $(0.1,0.3,0.6)$ & $2503 \pm 10$ & $921 \pm 10$ & \bm{$524 \pm 8$}\\
%DIF >  & $(0.1,0.6,0.3)$ & $2641 \pm 14$ & \bm{$833 \pm 5$} & $574 \pm 14$\\
%DIF >  & $(0.6,0.1,0.3)$ & \bm{$2470 \pm 9$} & $956 \pm 5$ & $573 \pm 8$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 10}} & $(0.33,0.33,0.33)$ & $1430 \pm 8$ & $630 \pm 4$& $205 \pm 2$\\
%DIF >  & $(0.1,0.3,0.6)$ & $1493 \pm 13$ & $615 \pm 4$ & $209 \pm 2$\\
%DIF >  & $(0.1,0.6,0.3)$ & $1542 \pm 10$ & \bm{$554 \pm 4$} & $229 \pm 5$\\
%DIF >  & $(0.6,0.1,0.3)$ & \bm{$1378 \pm 5$} & $663 \pm 6$ & $202 \pm 4$\\
%DIF > \hline
%DIF > \end{tabular}
%DIF > \caption{MO-PTSP results with different weight vectors. Values in bold indicate the best results on each map, when they are separated at least by $2$ standard errors from the others.}
%DIF > \label{tab:weights}
%DIF > \end{center}
%DIF > \end{table}
\DIFaddbegin 
...
\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{|p{0.8cm}|p{2.2cm}|m{1.13cm}|m{1.13cm}|m{1.13cm}|}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \bgroup
%DIF > \def\arraystretch{0.9}%  1 is the default, change whatever you need
...
\hline
 ...
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.3,0.6)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_2$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1657 \pm 8$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1657$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$130 \pm 2$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$130$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$773 \pm 11$}%%%
...
\hline
 ...
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.3,0.6)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_2$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1402 \pm 6$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1402$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$236 \pm 5$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$236$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$354 \pm 2$}%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{354} }\bm{$(<0.01)$}\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.6,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_3$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1416 \pm 8$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1416$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$219 \pm 4$} %%%
...
\hline
 ...
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.3,0.6)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_2$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1378 \pm 5$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1378$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$211 \pm 3$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$211$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$268 \pm 5$}%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{268} }\bm{$(<0.01)$}\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.6,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_3$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1385 \pm 6$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1385$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$203 \pm 4$} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{203} }\bm{$(<0.01)$} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$291 \pm 7$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$291$}\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$(0.6,0.1,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_4$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$1363 \pm 4$} %%%
...
\hline
 ...
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.3,0.6)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_2$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1385 \pm 7$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1385$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$304 \pm 4$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$304$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$542 \pm 4$}%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{542} }\bm{$(<0.01)$}\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.6,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_3$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1423 \pm 6$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1423 $ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$273 \pm 4$} %%%
...
\hline
 ...
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.6,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_3$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1467 \pm 9$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1467$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$411 \pm 5$} %%%
...
\hline
 ...
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.6,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_3$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1703 \pm 11$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1703$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$499 \pm 4$} %%%
...
\hline
 ...
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.3,0.6)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_2$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1475 \pm 10$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1475$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$602 \pm 5$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$602$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$243 \pm 6$}%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{243} }\bm{$(0.01)$}\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.6,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_3$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1489 \pm 12$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1489$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$549 \pm 3$} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{549} }\bm{$(<0.01)$} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$264 \pm 6$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$264$}\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$(0.6,0.1,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_4$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$1407 \pm 8$} %%%
...
\hline
 ...
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.3,0.6)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_2$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1804 \pm 10$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1804$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$269 \pm 4$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$269$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$357 \pm 4$}%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{357} }\bm{$(<0.01)$}\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.6,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_3$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1826 \pm 10$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1826$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$230 \pm 3$} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{230 }\bm{$(<0.01)$} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$392 \pm 7$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$392$}\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$(0.6,0.1,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_4$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$1732 \pm 9$} %%%
...
\hline
 ...
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.3,0.6)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_2$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$2503 \pm 10$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$2503$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$921 \pm 10$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$921$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$524 \pm 8$}%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{524} }\bm{$(<0.01)$}\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.6,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_3$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$2641 \pm 14$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$2641$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$833 \pm 5$} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{833} }\bm{$(<0.01)$} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$574 \pm 14$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$574$}\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$(0.6,0.1,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_4$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$2470 \pm 9$} %%%
...
\hline
 ...
 & \DIFdelbeginFL \DIFdelFL{$(0.1,0.6,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_3$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$1542 \pm 10$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1542$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$554 \pm 4$} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{554} }\bm{$(<0.01)$} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$229 \pm 5$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$229$}\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$(0.6,0.1,0.3)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$W_4$ }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \bm{$1378 \pm 5$} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textbf{1378} }\bm{$(<0.01)$} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$663 \pm 6$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$663$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$202 \pm 4$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$202$}\DIFaddendFL \\
\hline
\end{tabular}
\DIFaddbeginFL \egroup
\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{MO-MCTS results in }\DIFdelendFL MO-PTSP \DIFaddbeginFL \DIFaddFL{averages }\DIFaddendFL with different weight vectors. \DIFdelbeginFL \DIFdelFL{Significantly better results per map are }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Includes p-values for the objective with the highest weight. Results }\DIFaddendFL in bold \DIFaddbeginFL \DIFaddFL{are statistically significant (p-value $<0.05$)}\DIFaddendFL .}
\label{tab:weights}
\end{center}
\end{table}
...
%DIF > \bgroup
%DIF > %\def\arraystretch{0.9}%  1 is the default, change whatever you need
%DIF > \renewcommand\tabcolsep{5pt}
%DIF > \begin{tabular}{|p{0.9cm}|p{0.5cm}|m{1.9cm}|m{1.75cm}|m{1.75cm}|}
%DIF > \hline
%DIF > \textbf{Map}  & \textbf{$W$} & \textbf{Time} &  \textbf{Fuel} & \textbf{Damage} \\ 
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 1}} & $W_1$ & $1654$ & $131$& $846$\\
%DIF >  & $W_2$ & $1657$ & $130$ & \textbf{773} \bm{$(<0.01)$}\\
%DIF >  & $W_3$ & $1681$ & \textbf{131} $(0.72)$ & $837$\\
%DIF >  & $W_4$ & \textbf{1649} $(0.63)$ & $132$ & $833$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 2}} & $W_1$ & $1409$ & $235$& $364$\\
%DIF >  & $W_2$ & $1402$ & $236$ & $354$ $(0.1)$\\
%DIF >  & $W_3$ & $1416$ & \textbf{219} \bm{$(0.01)$} & $360$\\
%DIF >  & $W_4$ & \textbf{1396} $(0.55)$ & $245$ & $361$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 3}} & $W_1$ & $1373$ & $221$& $301$\\
%DIF >  & $W_2$ & $1378$ & $211$ & \textbf{268} $(0.053)$\\
%DIF >  & $W_3$ & $1385$ & \textbf{203} $(0.11)$ & $291$\\
%DIF >  & $W_4$ & \textbf{1363} $(0.17)$ & $229$ & $285$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 4}} & $W_1$ & \textbf{1383} & $291$& $565$\\
%DIF >  & $W_2$ & $1385$ & $304$ & \textbf{542} \bm{$(0.01)$}\\
%DIF >  & $W_3$ & $1423 $ & \textbf{273} \bm{$(0.01)$} & $583$\\
%DIF >  & $W_4$ & $1388$ $(0.74)$ & $309$ & $559$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 5}} & $W_1$ & $1405$ & $467$& $559$\\
%DIF >  & $W_2$ & $1431$ & $447$ & \textbf{541} $(0.23)$\\
%DIF >  & $W_3$ & $1467$ & \textbf{411} \bm{$(<0.01)$} & $567$\\
%DIF >  & $W_4$ & \textbf{1399} $(0.60)$ & $469$ & $547$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 6}} & $W_1$ & $1575$ & $549$& $303$\\
%DIF >  & $W_2$ & $1626$ & $540$ & \textbf{286} $(0.21)$\\
%DIF >  & $W_3$ & $1703$ & \textbf{499} \bm{$(<0.01)$} & $316$\\
%DIF >  & $W_4$ & \textbf{1571} $(0.68)$ & $559$ & $294$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 7}} & $W_1$ & $1434$ & $599$& $284$\\
%DIF >  & $W_2$ & $1475$ & $602$ & \textbf{243} \bm{$(0.01)$}\\
%DIF >  & $W_3$ & $1489$ & \textbf{549} \bm{$(<0.01)$} & $264$\\
%DIF >  & $W_4$ & \textbf{1407} \bm{$(0.01)$} & $618$ & $270$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 8}} & $W_1$ & $1761 $ & $254$& $382$\\
%DIF >  & $W_2$ & $1804$ & $269$ & 357 \bm{$(<0.01)$}\\
%DIF >  & $W_3$ & $1826$ & 230 \bm{$(<0.01)$} & $392$\\
%DIF >  & $W_4$ & \textbf{1732} \bm{$(0.02)$} & $311$ & $379$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 9}} & $W_1$ & $2501$ & $926$& $574$\\
%DIF >  & $W_2$ & $2503$ & $921$ & \textbf{524} \bm{$(<0.01)$}\\
%DIF >  & $W_3$ & $2641$ & \textbf{833} \bm{$(<0.01)$} & $574$\\
%DIF >  & $W_4$ & \textbf{2470} $(0.06)$ & $956$ & $573$\\
%DIF > \hline
%DIF >  \multirow{3}{*}{\textbf{Map 10}} & $W_1$ & $1430$ & $630$& $205$\\
%DIF >  & $W_2$ & $1493$ & $615$ & \textbf{209} $(0.16)$\\
%DIF >  & $W_3$ & $1542$ & \textbf{554} \bm{$(0.00)$} & $229$\\
%DIF >  & $W_4$ & \textbf{1378} \bm{$(<0.01)$} & $663$ & $202$\\
%DIF > \hline
%DIF > \end{tabular}
%DIF > \egroup
%DIF > \caption{MO-PTSP results with different weight vectors. Values in bold indicate the best results on each map, when they are separated at least by $2$ standard errors from the others.}
...
Extending this comparison to all maps, each pair of algorithms ends with a triplet $(D, \O, d)$\DIFdelbegin \DIFdel{, where }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend $D$ is the number of maps where the first algorithm dominates the second\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend $\O$ is the amount of maps where no dominance can be established, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{either because the Mann-Whitney-Wilcoxon non-parametric test failed, or because there is no dominance according to the dominance rules described in Section~\ref{sec:moo}. Finally, }\DIFaddend $d$ states the number of maps where the first is dominated by the second. \DIFaddbegin \DIFadd{For example, a triplet $(D, \O, d) = (8,2,0)$ comparing algorithms $A$ and $B$ would mean that the results obtained by $A$ dominate those from $B$ in $8$ of the $10$ maps, and that it is not possible to derive any dominance in the other $2$. }\DIFaddend Table~\ref{tab:resMOPTSP} summarizes these results for all \DIFaddbegin \DIFadd{the }\DIFaddend algorithms tested.
