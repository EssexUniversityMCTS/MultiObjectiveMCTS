...
One of the most well-known algorithms in the literature is the Non-dominated Sorting Evolutionary Algorithm II (NSGA-{II})~\cite{Deb2002}, \DIFaddbegin \DIFadd{the }\DIFaddend pseudocode of which is shown in Algorithm~\ref{alg:nsga2}. As in any evolutionary algorithm, NSGA-{II} evolves a set of $N$ individuals in a population denoted as $Pop$ with the difference that they are ranked according to \DIFaddbegin \DIFadd{a }\DIFaddend dominance criterion and a crowding distance measure\DIFdelbegin \DIFdel{which is }\DIFdelend \DIFaddbegin \DIFadd{, which are }\DIFaddend used to maintain a good diversity of solutions. \DIFaddbegin \DIFadd{After each iteration of the algorithm, only the best $N$ individuals according to this ranking are maintained to the next generation.
}\DIFaddend 
...
Here, if $\overline{r}$ is not dominated by the local Pareto front, it is added to the front and $\overline{r}$ is propagated to its parent. \DIFdelbegin \DIFdel{In case }\DIFdelend \DIFaddbegin \DIFadd{It may also happen that $\overline{r}$ dominates all or some solutions of the local front, in which case the new solution is included and the dominated solutions are removed from the Pareto front. If }\DIFaddend $\overline{r}$ is dominated by the node's Pareto front, the local Pareto front does not change and there is no need to \DIFdelbegin \DIFdel{keep }\DIFdelend \DIFaddbegin \DIFadd{maintain }\DIFaddend this propagation up the tree. Three observations can be made about the mechanism described here:
...
\DIFdelbegin \DIFdel{The same three algorithms have been tested in the MO-PTSP domain. The experiments are performed in the $10$ maps available within the framework~}\footnote{\DIFdel{Available at }%DIFDELCMD < \url{www.ptsp-game.net}%%%
}%DIFAUXCMD
...
\hline
\end{tabular}
\caption{MO-MCTS results in MO-PTSP with different weight vectors. \DIFdelbeginFL \DIFdelFL{Significantly better }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Values in bold indicate the best }\DIFaddendFL results \DIFdelbeginFL \DIFdelFL{per }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{on each }\DIFaddendFL map\DIFaddbeginFL \DIFaddFL{, when they }\DIFaddendFL are \DIFdelbeginFL \DIFdelFL{in bold}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{separated at least by $2$ standard errors from the others}\DIFaddendFL .}
\label{tab:weights}
\end{center}
\end{table}

\DIFaddbegin \DIFadd{The same three algorithms have been tested in the MO-PTSP domain. The experiments are performed in the $10$ maps available within the framework~}\footnote{\DIFadd{Available at }\url{www.ptsp-game.net}}\DIFadd{. In this case, the optimal Pareto front is not known in advance, and normally differs from one map to another. Hence, the mechanisms to compare the performance of the algorithms tested need to be different than the one used for DST.
}

\DIFadd{The idea is as follows. First of all, $4$ different weight vectors are tested: $W_1 = (0.33,0.33,0.33)$, $W_2 = (0.1,0.3,0.6)$, $W_3 = (0.1,0.6,0.3)$ and $W_4 = (0.6,0.1,0.3)$, where each $w_i$ corresponds to the weight for an objective ($w_t$ for time, $w_f$ for fuel and $w_d$ for damage, in this order). $W_1$ treats all objectives as having the same weight, while the other three give more relevance to different objectives. These vectors then provide a wide spectrum for weights in this benchmark. In this case, MO-MCTS uses these weights to select an action based on a weighted sum, which in preliminary experiments has shown better performance than the Euclidean distance mechanism.
}

\DIFadd{The first point to check is if the different weight vectors affect the solutions obtained by MO-MCTS. Table~\ref{tab:weights} shows the results of executing the MO-MCTS controller during $30$ games in each one of the $10$ maps of the MO-PTSP, for every weight vector. It can be seen that the highest weight in $W$ leads, in most of the cases, to the best solution in that objective in the map. 
}
...
These results suggest that the weights effectively work on making the algorithm converge to different points in the Pareto front discovered by the algorithm. Now, it is time to compare these results with the other algorithms. In order to do this, the same number of \DIFdelbegin \DIFdel{experiments is run }\DIFdelend \DIFaddbegin \DIFadd{runs is performed }\DIFaddend for NSGA-II and MCTS in the $10$ maps of the benchmark.
