...
Here, if $\overline{r}$ is not dominated by the local Pareto front, it is added to the front and $\overline{r}$ is propagated to its parent. It may also happen that $\overline{r}$ dominates all or some solutions of the local front, in which case the new solution is included and the dominated solutions are removed from the Pareto front. If $\overline{r}$ is dominated by the node's Pareto front, the local Pareto front does not change and there is no need to maintain this propagation up the tree. Three observations can be made about the mechanism described here:
...
\hline
\end{tabular}
\caption{MO-MCTS results in MO-PTSP with different weight vectors. Values in bold indicate the best results on each map, when they are separated at least by $2$ standard errors from the others.}
\label{tab:weights}
\end{center}
\end{table}
