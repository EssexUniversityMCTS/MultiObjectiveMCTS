...
\begin{figure} [!t]
	...
	\caption{Decision and objective spaces in a MOP with two variables ($x_1$ and $x_2$) and two objectives ($f_1$ and $f_2$). In the objective space, yellow dots are non-optimal objective vectors, while blue dots form a non-dominated set.}
	\label{fig:moop}
	\end{center}
\end{figure}
...
As mentioned before, the objective of this paper is to propose an MO-MCTS algorithm that is suitable for real-time domains. Although the work discussed here is influenced by Wang's approach, some modifications need to be made in order to overcome the high computational cost involved in their approach.
